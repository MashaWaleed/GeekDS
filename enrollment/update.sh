...
while true; do
	response="$(busybox wget -q -O - 192.168.1.11:5000/api/devices/apk/version)"
	del=$'\n\t{}'
	...
	[[ "$current" < "$version" ]] && logger "$(busybox wget -q -O /sdcard/app.apk 192.168.1.11:5000/api/devices/apk/latest 2>&1)" && logger "$(pm uninstall com.example.geekds 2>&1)" && logger "$(pm install -r -g /sdcard/app.apk 2>&1)"
	sleep 60s
done
